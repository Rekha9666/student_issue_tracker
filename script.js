  ...
  createUserWithEmailAndPassword(auth, email, password)
    .then(async (userCredential) => {
      ...
      await setDoc(doc(db, "users", user.uid), {
        email: user.email,
        role: "student"   // manually change to "mentor" in Firestore Console for mentors
      });
...
// Load Issues (only mentors will succeed due to Firestore rules)
async function loadIssues() {
  ...
  try {
    ...
    querySnapshot.forEach(docSnap => {
      ...
      div.textContent = `${issue.title} - ${issue.description}`;
      issuesList.appendChild(div);
    });
  } catch (e) {
    console.error("Error loading issues (likely not a mentor): ", e);
  }
}